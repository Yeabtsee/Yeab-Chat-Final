...
io.on("connection", (socket) => {
  ...
  socket.on("send_private_message", (data) => {
    const { message, receiverUsername, sender, timestamp, imageUrl } = data;
  ...
  socket.on("message_seen", async (data) => {
    ...
    try {
      ...
      console.log(
        "Searching for messages with timestamp between:",
        minTimestamp,
        "and",
        maxTimestamp
      );
      ...
      if (result.modifiedCount === 0) {
        console.error(
          "No message was updated. Timestamp mismatch still exists."
        );
        return;
      }
